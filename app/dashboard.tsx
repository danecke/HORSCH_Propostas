...
function QuotesView({ quotes, me, onChanged }: { quotes: Quote[]; me: CurrentAccess; onChanged: () => Promise<void> }) {
  ...
  return <div className="content-frame"><header className="page-heading"><div><span className="eyebrow">Fluxo de atendimento</span><h1>Cotações</h1><p>Concessionária solicita → dados recentes retornam automaticamente ou Gestão Global responde → Concessionária aprova ou reprova → Gestor Fábrica coloca o pedido.</p></div></header>
    {me.permissions.requestQuote && <form className="panel quote-request-panel" onSubmit={(event) => void submit(event)}><div><strong>Solicitar nova cotação</strong><span>Se o PN tiver cadastro completo imputado nos últimos 30 dias, descrição, VT, origem e net price retornam automaticamente.</span></div><div className="quote-request-fields"><label className="field"><span>PN</span><input value={pn} onChange={(event) => setPn(event.target.value)} placeholder="Ex.: 34061200" required /></label><button className="primary-button" disabled={saving}>{saving ? "Solicitando..." : "Solicitar cotação"}</button></div>{error && <p className="form-error">{error}</p>}</form>}
