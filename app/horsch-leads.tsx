...
export type LeadModuleData = { leads: Lead[]; ownerLeads?: Lead[]; leadDetails: Lead[]; byPartNumberStage: LeadPartInsight[]; customerMachines: LeadCustomerMachineInsight[]; metrics: LeadGroup; byDealership: LeadGroup[]; bySeller: LeadGroup[]; sellers: Array<{ email: string; name: string; dealershipId: number | null }>; dealerships?: Array<{ id: number; name: string }>; canEdit: boolean; metricsOnly: boolean; moduleEnabled?: boolean };
...
export function HorschLeadsView({ data, me, onChanged }: { data: LeadModuleData; me: CurrentAccess; onChanged: (message: string) => Promise<void> }) {
  ...
  const [leadTab, setLeadTab] = useState<"mine" | "all">("mine");
  ...
  const ownerLeads = data.ownerLeads ?? data.leads.filter((lead) => [lead.sellerEmail, lead.createdByEmail].some((email) => email?.trim().toLocaleLowerCase("pt-BR") === me.email.trim().toLocaleLowerCase("pt-BR")));
  const listSource = leadTab === "mine" ? ownerLeads : data.leads;
  const filtered = useMemo(() => {
    const normalized = term.trim().toLocaleLowerCase("pt-BR");
    return listSource.filter((lead) => !normalized || [lead.customerName, lead.phone, lead.email, lead.machineDomain, lead.partNumbers, lead.partsOfInterest, lead.sellerName, lead.lostReason].some((value) => value.toLocaleLowerCase("pt-BR").includes(normalized)));
  }, [listSource, term]);
  const groupedOwnerLeads = STAGE_ORDER.map((stage) => ({ stage, leads: filtered.filter((lead) => lead.stage === stage) })).filter((group) => group.leads.length);
  ...
  return <div className="content-frame leads-shell">
    ...
    {data.metricsOnly ? <MetricsOnly data={data} /> : <>
      <section className="panel leads-list-panel"><header className="panel-header"><div><h2>{leadTab === "mine" ? "Minha carteira de leads" : "Todos os leads da concessionária"}</h2><p>{leadTab === "mine" ? "Leads em que você é o responsável, separados por etapa do funil." : "Dados preenchidos pelos cargos da concessionária."}</p><div className="leads-view-tabs" role="tablist"><button type="button" className={leadTab === "mine" ? "active" : ""} onClick={() => setLeadTab("mine")}>Minha carteira ({ownerLeads.length})</button><button type="button" className={leadTab === "all" ? "active" : ""} onClick={() => setLeadTab("all")}>Todos ({data.leads.length})</button></div></div><label className="leads-search"><span>Buscar</span><input value={term} onChange={(event) => setTerm(event.target.value)} placeholder="Nome, máquina, peça ou vendedor" /></label></header>{filtered.length ? leadTab === "mine" ? <div className="lead-status-groups">{groupedOwnerLeads.map((group) => <section className="lead-status-group" key={group.stage}><header><div><span className={stageClass(group.stage)}>{STAGE_LABELS[group.stage]}</span><strong>{group.leads.length} lead{group.leads.length === 1 ? "" : "s"}</strong></div><span>{group.leads.reduce((sum, lead) => sum + lead.negotiatedValueCents, 0) ? money(group.leads.reduce((sum, lead) => sum + lead.negotiatedValueCents, 0)) : "Sem valor negociado"}</span></header><div className="lead-list">{group.leads.map((lead) => <LeadCard key={lead.id} lead={lead} onEdit={() => { setEditing(lead); setShowForm(true); }} />)}</div></section>)}</div> : <div className="lead-list">{filtered.map((lead) => <LeadCard key={lead.id} lead={lead} onEdit={() => { setEditing(lead); setShowForm(true); }} />)}</div> : <div className="leads-empty"><strong>{leadTab === "mine" ? "Nenhum lead na sua carteira" : "Nenhum lead registrado"}</strong><span>{term ? "Ajuste a busca ou cadastre uma nova oportunidade." : leadTab === "mine" ? "Ao cadastrar ou assumir um lead, ele aparecerá aqui separado pelo status." : "Cadastre o primeiro lead para iniciar o funil."}</span></div>}</section>
    </>}
    {showForm && <LeadModalEnhanced lead={editing} me={me} sellers={data.sellers} dealerships={data.dealerships ?? []} priceCatalog={priceCatalog} machineModels={machineModels.map((model) => model.name)} onClose={() => { setShowForm(false); setEditing(null); }} onSaved={async (message) => { setShowForm(false); setEditing(null); await onChanged(message); }} />}
  </div>;
}
...
function LeadModalEnhanced({ lead, me, sellers, dealerships, priceCatalog, machineModels, onClose, onSaved }: { lead: Lead | null; me: CurrentAccess; sellers: LeadModuleData["sellers"]; dealerships: NonNullable<LeadModuleData["dealerships"]>; priceCatalog: PriceCatalogItem[]; machineModels: string[]; onClose: () => void; onSaved: (message: string) => Promise<void> }) {
  ...
  const [dealershipId, setDealershipId] = useState(lead?.dealershipId ?? me.dealershipIds[0] ?? me.dealershipId ?? 0);
  ...
  async function submit(event: React.FormEvent) {
    ...
    const response = await fetch("/api/leads", { method: lead ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...(lead ? { id: lead.id } : { dealershipId }), customerName, phone, email, machineDomain: resolvedMachineModel, partNumbers: resolvedPartNumbers, partsOfInterest: resolvedFamilies, temperature, stage, negotiatedValueCents: parseMoney(negotiatedValue), invoiceNumber, invoiceValueCents: parseMoney(invoiceValue), lostReason, sellerName, sellerEmail }) });
