...
function LeadCard({ lead, onEdit }: { lead: Lead; onEdit: () => void }) {
  return <article className="lead-card"><header><div><span className="eyebrow">{lead.id}</span><h3>{lead.customerName}</h3><p>{lead.dealership} · Atualizado em {date(lead.updatedAt)}</p></div><span className={stageClass(lead.stage)}>{STAGE_LABELS[lead.stage]}</span></header><div className="lead-card-grid"><div><span>Contato</span><strong>{lead.phone || lead.email || "—"}</strong><small>{lead.email && lead.phone ? lead.email : ""}</small></div><div><span>Máquinas</span><strong>{lead.machineDomain || "—"}</strong></div><div><span>Peças de interesse</span><strong>{lead.partsOfInterest || "—"}</strong></div><div><span>Temperatura</span><strong className={`lead-temperature ${lead.temperature}`}>{TEMPERATURE_LABELS[lead.temperature]}</strong></div><div><span>Valor negociado</span><strong>{money(lead.negotiatedValueCents)}</strong></div><div><span>Vendedor</span><strong>{lead.sellerName || "—"}</strong><small>{lead.stage === "won" ? `NF ${lead.invoiceNumber || "não informada"}` : "Responsável comercial"}</small></div></div><footer><span>Próxima atualização no funil fica registrada no histórico do lead.</span><button className="outline-button compact" type="button" onClick={onEdit}>Editar</button></footer></article>;
}
