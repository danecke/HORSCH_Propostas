...
const STATUS_VALUES = ["N2 Elegível", "N3 Elegível", "N3 com Negociação", "Não Elegível", "Divergência de Preço", "NF Duplicada", "Produto/Estado/Cliente sem Cadastro", "Pendente Justificativa", "Aprovação Manual - Base de Cálculo"] as const;
type SaleStatus = (typeof STATUS_VALUES)[number];
type ReimbursementSegment = "N2" | "N3" | "normal";
type ParsedSale = { sourceRow: number; partNumber: string; description: string; quantity: number; costUnitCents: number; saleNetUnitCents: number; invoiceUnitCents: number; clientName: string; clientCnpj: string; invoiceNumber: string; state: string; dealershipName: string; dealershipCnpj: string };
...
function parseSales(bytes: Uint8Array, fileName: string): ParsedSale[] {
  ...
  const indexes = {
    ...
    dealership: headerIndex(headers, [/concession/, /dealer/]), dealershipCnpj: headerIndex(headers, [/cnpj.*concession/, /concession.*cnpj/, /cnpj.*dealer/]),
  };
  const labels: Record<keyof typeof indexes, string> = {
    partNumber: "PN", description: "Descrição", quantity: "Quantidade", cost: "Custo Médio Líquido Unitário", saleNet: "Valor Venda Líquido Unitário", invoice: "Valor Venda NF Unitário", client: "Cliente", document: "CPF/CNPJ", nf: "NF", state: "Estado", dealership: "Concessionário", dealershipCnpj: "CNPJ do Concessionário",
  };
  ...
  const parsed = rows.slice(1)
    .map((row, index) => ({
      ...
      dealershipName: indexes.dealership >= 0 ? String(row[indexes.dealership] ?? "").trim() : "", dealershipCnpj: indexes.dealershipCnpj >= 0 ? normalizeDocument(row[indexes.dealershipCnpj]) : "",
    }))
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const oldSales = allDealers.length
      ? await db.select().from(reimbursementSales).where(inArray(reimbursementSales.dealershipId, allDealers.map((dealer) => dealer.id)))
      : [];
    ...
    const existing = new Set(oldSales.map((row) => row.duplicateKey || `${row.dealershipId ?? 0}|${normalize(row.invoiceNumber)}|${normalize(row.partNumber)}`));
    ...
    for (const row of parsed) {
      ...
      const baseSource = hasNetPrice ? "NET_PRICE_VIGENTE" : "CUSTO_MEDIO_EXCEPCIONAL";
      ...
      let status: SaleStatus = "Não Elegível"; let reimbursement = 0; let negotiation = 0; let reasonCode = "";
      const history = oldSales.filter((item) => item.dealershipId === dealerId && normalize(item.partNumber) === normalize(row.partNumber) && item.liquidTotalCents > 0);
      const historicalMarginBps = history.length ? Math.round(history.reduce((sum, item) => sum + item.marginBps, 0) / history.length) : null;
      const marginVariationBps = historicalMarginBps === null ? null : marginBps - historicalMarginBps;
      if (duplicate) { status = "NF Duplicada"; reasonCode = "NF_DUPLICADA"; }
      else if (!client || !row.state) { status = "Produto/Estado/Cliente sem Cadastro"; reasonCode = "CADASTRO_AUSENTE"; }
      else if (!hasNetPrice) { status = "Aprovação Manual - Base de Cálculo"; reasonCode = "NET_PRICE_AUSENTE"; }
      else if (program === "N3") { if (price.n3 === null || Math.abs(row.invoiceUnitCents - price.n3) > n3ToleranceCents) status = "Divergência de Preço"; else { status = marginBps < 0 ? "N3 com Negociação" : "N3 Elegível"; reimbursement = marginBps < 0 ? 0 : Math.round(base * 0.07); negotiation = marginBps < 0 ? Math.max(0, costTotal - liquidTotal) : 0; } }
      else if (program === "N2") { if (marginBps < 0) { status = "Não Elegível"; reasonCode = "MARGEM_NEGATIVA_N2"; } else if (marginBps <= 2000) { status = "N2 Elegível"; reimbursement = Math.round(base * 0.04); } else status = "Não Elegível"; }
      if (status.endsWith("Elegível") && historicalMarginBps !== null && Math.abs(marginVariationBps ?? 0) > 1000) { status = "Pendente Justificativa"; reasonCode = "VARIACAO_MARGEM"; reimbursement = 0; }
...
export async function PATCH(request: Request) {
  const profile = await getAccessProfile(); if (!profile) return errorResponse("Acesso não autorizado.", 403);
  try { const body = await request.json() as { id?: number; lineId?: number; action?: string; note?: string; netPriceCents?: number }; if (body.lineId) { const lineId = Math.trunc(Number(body.lineId)); const lineDb = await ensureReimbursementStorage(); const [sale] = await lineDb.select().from(reimbursementSales).where(eq(reimbursementSales.id, lineId)).limit(1); if (!sale) return errorResponse("Linha não encontrada.", 404); const managers = isClientManager(profile); const dealerRows = await visibleDealers(lineDb, profile); if (sale.dealershipId && !dealerRows.some((d) => d.id === sale.dealershipId)) return errorResponse("Linha fora do escopo.", 403); const action = String(body.action ?? ""); const note = String(body.note ?? "").trim(); if (action === "submit_justification") { if (!isDealerScoped(profile) || note.length < 5) return errorResponse("Informe uma justificativa válida."); await lineDb.update(reimbursementSales).set({ justification: note, justificationStatus: "submitted" }).where(eq(reimbursementSales.id, lineId)); } else if (managers && (action === "manual_base_set_net_price" || action === "manual_base_approve_cost")) { const net = Math.trunc(Number(body.netPriceCents) || 0); if (action === "manual_base_set_net_price" && net <= 0) return errorResponse("Informe um Net Price válido."); const base = (action === "manual_base_set_net_price" ? net : sale.costAvgUnitCents) * sale.quantity; await lineDb.update(reimbursementSales).set({ netPriceUsedCents: action === "manual_base_set_net_price" ? net : sale.netPriceUsedCents, calculationBaseCents: base, baseSource: action === "manual_base_set_net_price" ? "NET_PRICE_MANUAL" : "CUSTO_MEDIO_APROVADO", baseStatus: "", status: sale.reimbursementProgram === "N3" ? "N3 Elegível" : "N2 Elegível", reimbursementCents: sale.reimbursementProgram === "N3" ? Math.round(base * .07) : Math.round(base * .04), reasonCode: "" }).where(eq(reimbursementSales.id, lineId)); } else if (managers && (action === "approve_justification" || action === "reject_justification")) { await lineDb.update(reimbursementSales).set({ status: action === "approve_justification" ? (sale.reimbursementProgram === "N3" ? "N3 Elegível" : "N2 Elegível") : "Não Elegível", reimbursementCents: action === "approve_justification" ? (sale.reimbursementProgram === "N3" ? Math.round(sale.calculationBaseCents * .07) : Math.round(sale.calculationBaseCents * .04)) : 0, justificationStatus: action === "approve_justification" ? "approved" : "rejected", reasonCode: action === "approve_justification" ? "" : "JUSTIFICATIVA_REJEITADA" }).where(eq(reimbursementSales.id, lineId)); } else return errorResponse("Ação de linha não reconhecida."); await lineDb.insert(reimbursementApprovals).values({ importId: sale.importId, lineId, action, note, actorEmail: profile.email, actorName: profile.name, createdAt: dateNow() }); return Response.json({ ok: true }); } const id = Math.trunc(Number(body.id) || 0); const action = String(body.action ?? "").trim(); const note = String(body.note ?? "").trim(); if (!id || !action) return errorResponse("Informe lote e ação."); const db = await ensureReimbursementStorage(); const [batch] = await db.select().from(reimbursementImports).where(eq(reimbursementImports.id, id)).limit(1); if (!batch) return errorResponse("Lote de reembolso não encontrado.", 404); const dealers = await visibleDealers(db, profile); if (batch.dealershipId && !dealers.some((dealer) => dealer.id === batch.dealershipId)) return errorResponse("Lote fora do escopo do usuário.", 403); const transitions: Record<string, { from: string[]; to: string; manager: boolean }> = { submit: { from: ["processed", "rejected"], to: "submitted", manager: false }, start_review: { from: ["submitted"], to: "under_review", manager: true }, approve: { from: ["submitted", "under_review"], to: "approved", manager: true }, reject: { from: ["submitted", "under_review"], to: "rejected", manager: true }, mark_paid: { from: ["approved"], to: "paid", manager: true } }; const transition = transitions[action]; if (!transition || !transition.from.includes(batch.status)) return errorResponse("Essa ação não está disponível para o status atual do lote."); if (transition.manager && !isManager(profile)) return errorResponse("Somente Gestão Global, ADM e Gestor Fábrica podem analisar reembolsos.", 403); if (action === "reject" && note.length < 5) return errorResponse("Informe o motivo da rejeição."); const now = dateNow(); await db.update(reimbursementImports).set({ status: transition.to, decisionNote: note || batch.decisionNote, decidedByEmail: profile.email, decidedAt: transition.manager ? now : batch.decidedAt, updatedAt: now }).where(eq(reimbursementImports.id, id)); await db.insert(reimbursementApprovals).values({ importId: id, action, note, actorEmail: profile.email, actorName: profile.name, createdAt: now }); await recordAudit(db, { actorEmail: profile.email, actorName: profile.name, action: `reimbursement_${action}`, entity: "reimbursement_import", details: `Lote ${id}: ${transition.to}.`, before: { status: batch.status }, after: { status: transition.to, note } }); return Response.json({ ok: true, status: transition.to }); } catch (error) { return errorResponse(error instanceof Error ? error.message : "Não foi possível atualizar o workflow do reembolso.", 500); }
}
