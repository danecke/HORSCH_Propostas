...
export const proposals = sqliteTable(
  "proposals",
  {
    ...
    requestedNetPriceCents: integer("requested_net_price_cents"),
    claimedByEmail: text("claimed_by_email").notNull().default(""),
    claimedAt: text("claimed_at"),
    pdfVisualized: integer("pdf_visualized", { mode: "boolean" }).notNull().default(false),
    rejectionReason: text("rejection_reason").notNull().default(""),
    erpOrderNumber: text("erp_order_number").notNull().default(""),
    officialPdfPath: text("official_pdf_path").notNull().default(""),
    factoryDescription: text("factory_description").notNull().default(""),
    factoryVt: text("factory_vt").notNull().default(""),
    factoryOrigin: text("factory_origin").notNull().default(""),
    factoryNcm: text("factory_ncm").notNull().default(""),
    offerNetPriceCents: integer("offer_net_price_cents"),
    offerInvoiceUnitPriceCents: integer("offer_invoice_unit_price_cents"),
    offerValidUntil: text("offer_valid_until"),
...
export const reimbursementImports = sqliteTable("reimbursement_imports", {
  ...
  toleranceBps: integer("tolerance_bps").notNull().default(500),
...
export const reimbursementSales = sqliteTable("reimbursement_sales", {
  ...
  status: text("status").notNull().default("Não Elegível"),
  ...
  duplicateKey: text("duplicate_key").notNull().default(""),
  baseSource: text("base_source").notNull().default("NET_PRICE_VIGENTE"),
  baseStatus: text("base_status").notNull().default(""),
  reasonCode: text("reason_code").notNull().default(""),
  historicalMarginBps: integer("historical_margin_bps"),
...
export const reimbursementApprovals = sqliteTable("reimbursement_approvals", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  importId: integer("import_id").notNull().references(() => reimbursementImports.id, { onDelete: "cascade" }),
  lineId: integer("line_id").references(() => reimbursementSales.id, { onDelete: "cascade" }),
  action: text("action").notNull(),
